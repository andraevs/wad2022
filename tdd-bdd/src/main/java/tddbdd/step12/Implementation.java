...
final class NoAccessControlGateway implements AccessControlGateway {
    @Override
...
final class RoomBookingService {
    ...
    RoomBookingService() {
        this(new NoAccessControlGateway());
    }
    ...
    BookingResult book(BookingRequest request) {
        if (!policy.allows(request) || !schedule.isAvailableFor(request)) {
